...
- 栈：方法运行时使用的内存，比如main方法运行，进入方法栈中执行，执行完毕后出栈 （存储操作）
- 堆：存储对象或者数组，new来创建的，都存储在堆内存 （存储数据）
...
代码会从main方法开始执行，从上往下依次执行，看到方法进入方法，执行完毕后回到调用处。

**实参**是方法中调用的参数，**形参**是方法定义的参数。调用方法时形参和实参数据类型要保持一致。

确定是否要添加形参的小技巧：

- 我要干什么？这个问题是关于我们如何设置方法体的
- 我需要什么才能实现？这个问题是关于我们需不需要设置形参，设置几个形参的
- 是否返回？这个问题是关于要不要有返回值的

方法返回值的作用：方法是帮助我们解决实际问题的工具，但是通常情况下实际问题需要多个方法共同完成，也会出现下一个方法需要上一个方法的运行结果的情况。如果没有返回值的话，我们只能在控制台看到方法的运行结果而不能直接用使用这个方法的运行结果进行更多的操作。

void表示无返回值，想要有返回值把返回值的类型加在方法名之前就好了。

- 注意：方法应该只包含一个项目中**多次重复的固定的内容**，不要把会出现很多变化的内容写到方法里，不然后期使用这个方法的时候会多出很多不该有的限制条件。要明白方法应当只是一个零件而不是一个完整的机器。

### 6.2方法的重载

同一个类中，方法名相同，参数不同的方法，和返回值无关。参数不同：**个数不同**，**类型不同**，**顺序不同**。

## 结语

基础的Java语法方面到这里也就基本上结束了，总体来看语法和c相比差不多但是要比python繁琐一些。熟练掌握这些基础的知识可以为接下来对于数据结构的学习和软件设计模式的学习有很大的帮助。

这些更加深入的学习我会单独再开一个项目进行描述。
